...
ux_motion is at **1.2.1**. Three version numbers exist on purpose. Do not collapse them.
...
|---|---|---|---|
| **Library / API** | `__version__`, `API_VERSION`, `CONTRACT["api"]`, `pyproject.toml`, JS `UxMotion.version`, `PLAYER_VERSION` | **1.2.1** | Python facade + aligned browser player |
| **Plan IR** | `IR_VERSION`, plan field `v`, `CONTRACT["ir"]` | **1** | JSON wire shape for plans |
| **Package release** | Git tag / zip name / PyPI | **1.2.1** | Same as library for this project |

```mermaid
flowchart TB
  subgraph Library["Library semver 1.2.1"]
    API["API_VERSION"]
    PY["__version__"]
    PL["PLAYER_VERSION"]
...
```
Git tag:     v1.2.1
Zip:         ux_motion-1.2.1-complete.zip
